...
export default function FeatureCard({ icon: Icon, title, description, index = 0 }) {
  return (
    <motion.div
      ...
      className="group relative rounded-2xl sm:rounded-3xl bg-white/80 dark:bg-white/[0.04] border border-secondary/15 dark:border-white/10 p-6 sm:p-7 shadow-[0_10px_30px_-10px_rgba(28,28,28,0.08)] transition-all duration-300 hover:shadow-[0_20px_45px_-12px_rgba(184,133,15,0.25)] hover:border-secondary/40 dark:hover:border-accent/40 backdrop-blur-xl overflow-hidden flex flex-col justify-between"
    >
      ...
      <div className="relative z-10">
        <motion.div
          ...
          className="mb-4 sm:mb-5 flex h-12 w-12 sm:h-13 sm:w-13 items-center justify-center rounded-xl sm:rounded-2xl bg-secondary/10 dark:bg-accent/15 text-secondary dark:text-accent text-xl sm:text-2xl group-hover:bg-gradient-to-br group-hover:from-secondary group-hover:to-accent group-hover:text-white transition-all duration-300 shadow-sm"
        >
          <Icon />
        </motion.div>

        <h3 className="mb-2 text-lg sm:text-xl font-bold text-primary dark:text-paper tracking-tight group-hover:text-secondary dark:group-hover:text-accent transition-colors duration-200">
          {title}
        </h3>
        
        <p className="text-sm sm:text-[14px] leading-relaxed text-primary/70 dark:text-paper/70">
          {description}
        </p>
      </div>

      {/* Bottom subtle indicator tag */}
      <div className="relative z-10 mt-5 pt-3 border-t border-primary/5 dark:border-white/5 flex items-center justify-between text-xs font-semibold text-secondary dark:text-accent">
